// object.c — Content-addressable object store // v4
//
...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    uint8_t *full_object = malloc(total);
    ...
    compute_hash(full_object, total, id_out);

    // Step 5: Deduplication — if already exists, skip
    ...
    // Step 7: Build final and temp paths
    char final_path[512];
    ...
    ssize_t written = write(fd, full_object, total);
    free(full_object);
    ...
    // Step 9: fsync temp file — flush to disk
    fsync(fd);
    close(fd);

    // Step 10: Atomically rename temp → final
    if (rename(tmp_path, final_path) != 0) {
        unlink(tmp_path);
        return -1;
    }

    // Step 11: fsync the shard directory
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 6: Copy data portion and return
    ...
    memcpy(data_copy, buf + data_offset, data_len);
