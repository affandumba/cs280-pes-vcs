// commit.c — Commit creation and history traversal // v2
//
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 3: Read parent commit if exists
    ObjectID parent_id;
    ...
    void *buf;
    size_t buf_len;
    if (commit_serialize(&c, &buf, &buf_len) != 0) return -1;

    int rc = object_write(OBJ_COMMIT, buf, buf_len, commit_id_out);
    free(buf);
    if (rc != 0) return -1;
